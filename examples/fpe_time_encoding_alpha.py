"""Encode SSVEP CSV recordings using FHRR.fpe with larger time scaling (alpha=0.5).

Saves encoded hypervectors per window to recordings/encoded_fpe_alpha.npz
"""
import os
import sys
import glob
...
def main():
    ...
    for csv in csvs:
        ...
        for start, end, win in windows_from_data(ch_data, timestamps, fs, win_s, step_s):
            ...
            pvec = np.log1p(powers)
            pvec = pvec / (np.linalg.norm(pvec) + 1e-12)

            # INCREASED time scaling (alpha=0.5)
            time_t = float(center_time - timestamps[0])
            time_hv = fhrr.fpe(time_base, 0.5 * time_t)
    ...
    out_path = os.path.join(recordings_dir, 'encoded_fpe_alpha.npz')
